...
docker build -f Dockerfile.local -t mycompanyname/test1-db-migrator:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-service-identity:$version .

### ADMINISTRATION-SERVICE
Write-Host "*** BUILDING ADMINISTRATION-SERVICE ***" -ForegroundColor Green
Set-Location $administrationServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-service-administration:$version .

### SAAS-SERVICE
Write-Host "*** BUILDING SAAS-SERVICE ***" -ForegroundColor Green
Set-Location $saasServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-service-saas:$version .

### PRODUCT-SERVICE
Write-Host "*** BUILDING PRODUCT-SERVICE ***" -ForegroundColor Green
Set-Location $productServiceFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-service-product:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-gateway-web:$version .

### PUBLICWEB-GATEWAY
Write-Host "*** BUILDING WEB-PUBLIC-GATEWAY ***" -ForegroundColor Green
Set-Location $webPublicGatewayFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-gateway-web-public:$version .
...
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-app-authserver:$version .

### PUBLIC-WEB
Write-Host "*** BUILDING WEB-PUBLIC ***" -ForegroundColor Green
Set-Location $publicWebFolder
dotnet publish -c Release
docker build -f Dockerfile.local -t mycompanyname/test1-app-public-web:$version .
### Angular WEB App
if (Test-Path -Path $angularAppFolder) {
    ...
    # ng build --prod
    npm run build:prod
    docker build -f Dockerfile.local -t mycompanyname/test1-app-angular:$version .
}
